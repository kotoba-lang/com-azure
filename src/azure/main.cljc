(ns azure.main
  "Kotodama WASM entrypoint for the Azure clean-room actor (L4) — Clojure port.
  ...
  facts (`azure.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"azure.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "azure-compat" :tier tier :entities entities} 200])
